...
for o in np.unique(order_sci):
   ...
   #fitting low order function to flat spectrum for rejecting outliers
   fit_order=1
...
#   show()
   ...
   for wv in np.arange(wave_min,wave_max,step):
      i_flat=np.where((wave_flat >= wv) & (wave_flat <= (wv+step)) & (order_flat==o) & (residuals < mean_res+6.*std_res) ) #check which elements of array wave_flat are within spectral range "wv" and "wv+step"
      if len(i_flat[0])==0:
         i_flat=np.where((wave_flat >= wv) & (wave_flat <= (wv+step)) & (order_flat==o) & (residuals < mean_res+50.*std_res) )
...
#      int_cal=integrate.simps(flux_cal[i_cal],x=wave_cal[i_cal])
 ...
 #  print i_flat,i_fit,"AAAAAAAAAAAA"
   if len(i_fit[0])==0:
      i_fit=np.where(residuals < 1e9) 

   #another fit to ratios
   fit_order=6
...
#   plot(wave_sci[order_i],fitted[order_i])
 #  show()
   ...
   #applying the blaze correction to flat
   flux_corr=flux_flat/fitted

   #recalculating correction with use of corrected data
   fit_ratio2=[]
   for wv in np.arange(wave_min,wave_max,step):
      int_flux=np.median(flux_corr[i_flat])
      i_cal=np.where((wave_cal >= wv) & (wave_cal <= (wv+step)) )  #check which elements of array wave_flat are within spectral range "wv" and "wv+step"
      int_cal=np.median(flux_cal[i_cal])
      fit_ratio2.append(int_flux/int_cal)

   fit_ratio2=np.array(fit_ratio2)  


   #another fit to ratios
   fit_order=6
   coefficients=np.polyfit(fit_wave[i_fit], fit_ratio2[i_fit], fit_order)
   func=np.poly1d(coefficients)
   fitted2=np.polyval(func,wave_sci)



   scatter(fit_wave,fit_ratio2,c='r',edgecolor='None')
   scatter(fit_wave[i_fit], fit_ratio2[i_fit],c='g',edgecolor='None')
   plot(wave_sci[order_i],fitted2[order_i])
   show()
